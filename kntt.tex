    ...
    \foreach \startcount in {0,...,\startrange} 
      {
        ...
        \foreach \lcnt in {\lcntstart,...,\lcntend}
        {
            ...
            \pgfmathtruncatemacro{\cb}{255/(\startrange+1)*(\startrange-\startcount)}
            \definecolor{currentColor}{RGB}{\cr,\cg,\cb};
            \draw [currentColor, line width = 13, loosely dashed] (\level\l) -- (\nextlevel\h);
            \draw [currentColor, line width = 13, loosely dashed] (\nextlevel\l) -- (\level\h);
        }
      } 
  }
